...
// On Mainnet
const CONTRACT_ADDRESS = "0xaDe2e84eFd92A00854439f0394419cce1f85eD2E";
const Network = 'http://localhost:7545';
const Block_Explore="http://localhost:7545/"
window.ABI =[
	{
		"anonymous": false,
		"inputs": [
...
function print_info(result, is_verified) {
  ...
  if (!is_verified) {
    ...
    document.getElementById("student-document").data="http://dweb.link/ipfs/"+result[3];
...
async function sendHash() {
  ...
  $("#upload_file_button").attr("disabled", true);
    ...
    node = await Ipfs.create({repo: 'Aslam' + Math.random()});
...
function printTransactions(data, length) {
  ...
  for (let i = 0; i < length; i++) {
      ...
      image.data = `http://dweb.link/ipfs/${data[i].returnValues[1]}`;
